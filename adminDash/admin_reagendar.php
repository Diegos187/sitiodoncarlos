...
// Verificamos que se hayan recibido los datos necesarios desde el formulario
if (isset($_POST['id_form']) && isset($_POST['fecha']) && isset($_POST['horario'])) {
    $id_form = $_POST['id_form']; // ID de la cita a reagendar
    $fecha = $_POST['fecha']; // Nueva fecha solicitada
    $id_horario_nuevo = $_POST['horario']; // Nuevo horario solicitado
    ...
    $stmt_actual->bind_param('i', $id_form); // Enlazamos el ID de la cita como parámetro
    $stmt_actual->execute();
    $result_actual = $stmt_actual->get_result();
    $horario_actual = $result_actual->fetch_assoc()['id_horario']; // Extraemos el horario actual de la cita
    ...
    $stmt_disponible->bind_param('i', $horario_actual); // Enlazamos el ID del horario actual
    ...
    if ($stmt_reagendar->execute()) {
        ...
        $stmt_reservado->bind_param('i', $id_horario_nuevo); // Enlazamos el ID del nuevo horario
        $stmt_reservado->execute();

        // Obtener detalles de la cita y del cliente
        // Obtener los detalles de la cita, el cliente, el servicio y el producto asociados
        $query_cita = "
        ...
        $stmt_cita->bind_param('ii', $id_horario_nuevo, $id_form); // Enlazamos el nuevo horario y el ID de la cita
        $stmt_cita->execute();
        $result_cita = $stmt_cita->get_result();
        $cita = $result_cita->fetch_assoc(); // Obtenemos todos los detalles de la cita
