...
// Comprobamos que el usuario haya iniciado sesión y tenga el rol de administrador
...
if (isset($_POST['id_form']) && isset($_POST['nuevo_estado'])) {
    $id_form = $_POST['id_form']; // ID de la cita a actualizar
    $nuevo_estado = $_POST['nuevo_estado']; // Nuevo estado a establecer en la cita

    // Obtener detalles de la cita y del cliente
    // Consultamos la información actual de la cita (nombre del cliente, correo, estado actual, etc.)
    // Definimos la consulta SQL para obtener la información de la cita junto con los detalles del horario asociado.
    // La consulta selecciona el ID de formulario (cita), nombre, apellido, correo, estado actual de la cita,
    // y, si está disponible, la fecha y hora del horario reservado en la cita.
    $query_cita = "
    ...
    // Actualizar el estado de la cita en bd
    $query = "UPDATE Formulario SET estado = ? WHERE id_form = ?";
    $stmt = $conex->prepare($query);
    $stmt->bind_param('si', $nuevo_estado, $id_form);

    if ($stmt->execute()) {
        // Si el nuevo estado es 'cancelado', liberamos el horario asociado a la cita
        ...
        if ($nuevo_estado === 'cancelado') {
            ...
            $mensaje_cliente = "
                ...
                <p>Por favor, esté atento a nuevas actualizaciones por correo electrónico o su cuenta.</p>
                <p>Saludos cordiales,<br>Centro Técnico DC</p>
            ";
